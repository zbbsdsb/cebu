...
bool Persistence::save_refinement(const SimplicialComplexRefinement<LabelType>& complex,
    ...
    if (format == FileFormat::BINARY) {
        // Binary serialization not yet implemented, fall back to JSON
        auto json = JsonSerializer::serialize_refinement(complex);
        std::string json_str = JsonSerializer::pretty_print(json, 2);
        std::vector<uint8_t> data(json_str.begin(), json_str.end());
...
LoadResult<SimplicialComplexLabeled<LabelType>> Persistence::load_labeled(
    ...
    try {
        if (format == FileFormat::BINARY) {
            ...
            result.complex = JsonSerializer::template deserialize_labeled<LabelType>(json_obj);
            result.success = true;
            result.metadata = create_metadata(result.complex, data.size());
        } else if (format == FileFormat::JSON) {
            auto json_obj = nlohmann::json::parse(read_json_file(filename));
            result.complex = JsonSerializer::template deserialize_labeled<LabelType>(json_obj);
            result.success = true;
...
LoadResult<SimplicialComplexNarrative<LabelType>> Persistence::load_narrative(
    ...
    try {
        if (format == FileFormat::BINARY) {
            ...
            std::string json_str(decompressed_data.begin(), decompressed_data.end());
            auto json_obj = nlohmann::json::parse(json_str);
            result.complex = JsonSerializer::template deserialize_narrative<LabelType>(json_obj);
            result.success = true;
            result.metadata = create_metadata(result.complex, data.size());
        } else if (format == FileFormat::JSON) {
            auto json_obj = nlohmann::json::parse(read_json_file(filename));
...
LoadResult<SimplicialComplexRefinement<LabelType>> Persistence::load_refinement(
    ...
    try {
        if (format == FileFormat::BINARY) {
            ...
            std::string json_str(decompressed_data.begin(), decompressed_data.end());
            auto json_obj = nlohmann::json::parse(json_str);
            result.complex = JsonSerializer::template deserialize_refinement<LabelType>(json_obj);
            result.success = true;
            result.metadata = create_metadata(result.complex, data.size());
        } else if (format == FileFormat::JSON) {
            auto json_obj = nlohmann::json::parse(read_json_file(filename));
